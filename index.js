...
for (var i in ENswearWord){
    if (message.content.toLowerCase().includes(ENswearWord[i].toLowerCase())) foundInTextWord = true;
}

if (foundInTextWord){
    message.delete();
    message.channel.send(`<@${message.author.id}> Dont use swear words! Also... NO MEMES IN GENERAL`)
        message.delete({ timeout: 10000 })
}
    
    let channel = message.guild.channels.cache.find((x) => (x.name === "vilon" || x.name === "vilon-logs"))
    
    
    if(!channel) {
      return message.channel.send("there is no channel with name - vilon-logs or vilon.")
    }
                                                    
    
    let embed = new MessageEmbed()
    .setAuthor("Swear Word By: " + message.author.tag, message.author.avatarURL())
    ...
    channel.send(embed).then(m => {
      m.react("https://cdn.discordapp.com/emojis/734839401699606548.png?v=1")
    })
